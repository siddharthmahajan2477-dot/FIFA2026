...
# Service functions mirroring matches router logic

async def get_matches(db) -> List[Dict[str, Any]]:
    """Return mock tournament matches."""
    return [
        {
            "id": "m101",
            "home_team": "USA",
...
async def get_live_match(db) -> Dict[str, Any]:
    """Return mock live match data (fallback if external service fails)."""
    return {
        "id": "m101",
...
async def get_kickoff_countdown() -> Dict[str, Any]:
    return {
...
async def get_teams(db) -> List[Dict[str, Any]]:
    return [
...
async def get_standings(db) -> List[Dict[str, Any]]:
    return [
        {"group": "Group A", "teams": [
...
async def get_players(db) -> List[Dict[str, Any]]:
    return [
...
async def get_topscorers(db) -> List[Dict[str, Any]]:
    return [
